...
/// This crate has a runtime dependency on artifacts produced by the `unix-sniffer-ebpf` crate.
...
fn main() {
    ...
    let ebpf_package = packages
        .into_iter()
        .find(|Package { name, .. }| name == "unix-sniffer-ebpf")
        .unwrap();
    ...
    if build_ebpf {
        ...
        // We have a build-dependency on `unix-sniffer-ebpf`, so cargo will automatically rebuild us
        // if `unix-sniffer-ebpf`'s *library* target or any of its dependencies change. Since we
        // depend on `unix-sniffer-ebpf`'s *binary* targets, that only gets us half of the way. This
        ...
        let ebpf_target_dir = out_dir.join("unix-sniffer-ebpf");
